	...
	:QThread(parent),control_sock(control), Model(UpDownList_Model), port(listenport)
{
	...
	if (Mode == DownloadMode)
	{
		...
		{
			QMessageBox::information(NULL, QString::fromLocal8Bit("文件写错误"), QString::fromLocal8Bit("%1写错误").arg(Dest));
			return;
		}
	}
	else
	{
		...
		{
			QMessageBox::information(NULL, QString::fromLocal8Bit("文件读错误"), QString::fromLocal8Bit("%1读错误").arg(Sour));
			return;
		}
	}
	if (StatusFlag)
